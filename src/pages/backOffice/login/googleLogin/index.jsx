...
const GoogleLogin = () => {
  ...
  const { mutate: postGoogleLogin } = masterService.useMutationLoginSocial(
    (res) => {
      AlertClosed();
      if (res?.success) {
  ...
  const handleLogin = useGoogleLogin({
    flow: "auth-code",
    onSuccess: (res) => {
      AlertLoading({ text: "Signing in with Google..." });
      const code = res.code;
      postGoogleLogin({ payload: { type: "google", token: code } });
    },
    onError: () => {
      AlertError({ text: "Google Login failed. Please try again." });
    },
  });

  return (
    <div style={{ textAlign: "center" }}>
      <button className="sso-btn google-btn w-full" onClick={handleLogin}>
        <div>
