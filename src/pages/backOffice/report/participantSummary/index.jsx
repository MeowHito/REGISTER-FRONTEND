...
/** Every participant of the event in the period, whatever their payment status, with each one's money. */
export default function ParticipantSummary({ filters }) {
  const { t } = useTranslation();
  const tr = (k, o) => t(`back.report.ui.participants.${k}`, o);
  ...
  const { data, isFetching, refetch } = backOfficeServices.useQueryGetRegistrantSummary({
  ...
  const { mutateAsync: downloadExcel } = fileService.useMutationDownloadSummaryRegistrantExcel();
  ...
  const searchFields = ["fullName", "orderId", "transactionId", "eventTypeName", "paymentMethod", "paymentStatus"]
  ...
  return (
    <div className="flex flex-col">
      ...
      {!ready ? (
        ...
        >
          <OrderRowsTable rows={rows} total={total} page={page} pageSize={pageSize} loading={isFetching} onChange={handleTableChange} showStatus />
        </Panel>
      )}
    </div>
  );
